...
A modern MCP (Model Context Protocol) server with real-time UI for cross-client persistent memory, powered by the Longbow distributed vector database and its official Python SDK. - **MCP Protocol**: Full MCP implementation with stdio and SSE transports
